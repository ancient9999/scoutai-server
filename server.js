...
// Contact form submission — stores in memory (upgrade to email service later)
const contactMessages = [];
app.post("/contact", (req, res) => {
  const { name, email, company, message, type } = req.body;
  if (!name || !email || !message) return res.status(400).json({ error: "Name, email and message required" });
  contactMessages.push({ name, email, company, message, type, timestamp: new Date().toISOString() });
  console.log("New contact:", { name, email, company, type, message });
