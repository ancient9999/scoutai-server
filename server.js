...
const LEAGUES = {
  ...
  FAC:  { id:45,  name:"FA Cup",              country:"England",     flag:"🏴󠁧󠁢󠁥󠁮󠁧󠁿", season:2025 },
  EFL:  { id:48,  name:"EFL Cup",             country:"England",     flag:"🏴󠁧󠁢󠁥󠁮󠁧󠁿", season:2025 },
  CDR:  { id:143, name:"Copa del Rey",        country:"Spain",       flag:"🇪🇸", season:2025 },
  DFB:  { id:81,  name:"DFB Pokal",           country:"Germany",     flag:"🇩🇪", season:2025 },
  CPI:  { id:137, name:"Coppa Italia",        country:"Italy",       flag:"🇮🇹", season:2025 },
  CDF:  { id:66,  name:"Coupe de France",     country:"France",      flag:"🇫🇷", season:2025 },
  ...
  try {
    ...
    for (const pred of pending) {
      try {
        ...
        let status = "lost";
        const predResult = pred.result || "";

        if (predResult === "Home Win" || predResult === "Away Win" || predResult === "Draw") {
          // Standard result market
          status = actualResult === predResult ? "won" : "lost";
        } else if (predResult === "Over 2.5 Goals") {
          status = totalGoals > 2.5 ? "won" : "lost";
        } else if (predResult === "Under 2.5 Goals") {
          status = totalGoals < 2.5 ? "won" : "lost";
        } else if (predResult === "Both Teams to Score") {
          status = (homeGoals > 0 && awayGoals > 0) ? "won" : "lost";
        } else if (predResult === "Clean Sheet") {
          status = (homeGoals === 0 || awayGoals === 0) ? "won" : "lost";
        } else {
          // Fallback - compare with actual result
          status = actualResult === predResult ? "won" : "lost";
        }

        console.log(`Market check: predicted "${predResult}", actual result "${actualResult}", score ${homeGoals}-${awayGoals}, total goals ${totalGoals} → ${status}`);
