...
// ── DAILY CRON — hit this from cron-job.org at 8am every day ──────────────
// Generates POTD + parlays + emails all subscribers
app.get("/cron/daily", async (req, res) => {
  ...
  const results = { potd: null, parlays: null, emailsSent: 0, errors: [] };
  ...
  try {
    ...
    results.parlays = await parlayRes.json();
  } catch(e) { results.errors.push("Parlays: " + e.message); }

  try {
    // 3. Check pending results
    await checkPendingResults();
    results.resultsChecked = true;
  } catch(e) { results.errors.push("Results: " + e.message); }

  // 4. Email subscribers with daily predictions
  if (subs.length > 0 && results.potd && process.env.RESEND_API_KEY) {
    const potd = results.potd;
    const parlaySlip = results.parlays?.safe;

    const parlaySection = parlaySlip ? `
      <h2 style="color:#7c3aed;margin-top:24px;">🎯 Today's Safe Parlay (${parlaySlip.totalOdds}x odds)</h2>
      ${parlaySlip.picks.map(p => `
        <div style="padding:10px 14px;background:#f8f9fa;border-radius:8px;margin-bottom:8px;border-left:3px solid #7c3aed;">
          <strong>${p.home} vs ${p.away}</strong><br>
          <span style="color:#7c3aed;">Prediction: ${p.result}</span> · <span style="color:#64748b;">${p.confidence}% confidence</span>
        </div>
      `).join("")}
      <p style="color:#64748b;font-size:12px;">Combined confidence: ${parlaySlip.combinedConf}%</p>
    ` : "";
    ...
    const html = `
      ...
      <body style="font-family:sans-serif;max-width:560px;margin:0 auto;padding:20px;">
        <div style="background:linear-gradient(135deg,#0f172a,#1e293b);border-radius:16px;padding:24px;margin-bottom:24px;text-align:center;">
          <h1 style="color:#fff;margin:0;font-size:28px;">⚽ Scout<span style="color:#22c55e;">AI</span></h1>
          <p style="color:#64748b;margin:6px 0 0;font-size:12px;letter-spacing:2px;">DAILY PREDICTIONS</p>
        </div>

        <h2 style="color:#ea580c;">🔥 Prediction of the Day</h2>
        <div style="background:#fff7ed;border-radius:12px;padding:18px;border:1px solid #fed7aa;margin-bottom:16px;">
          <p style="color:#94a3b8;font-size:12px;margin:0 0 8px;">${potd.compName?.toUpperCase() || "TOP LEAGUE"}</p>
          <h3 style="margin:0 0 8px;">${potd.home} vs ${potd.away}</h3>
          <div style="display:inline-block;padding:6px 16px;border-radius:20px;background:#fff;border:1.5px solid #ea580c;color:#ea580c;font-weight:bold;margin-bottom:8px;">${potd.prediction?.result || "Prediction unavailable"}</div>
          <p style="color:#64748b;font-size:13px;margin:6px 0 0;">${potd.prediction?.result_confidence || 0}% confidence${potd.prediction?.score ? ` · Score: ${potd.prediction.score}` : ""}</p>
          ${potd.prediction?.reasoning ? `<p style="color:#475569;font-size:13px;margin:10px 0 0;line-height:1.6;">${potd.prediction.reasoning}</p>` : ""}
        </div>

        ${parlaySection}

        <div style="background:#0f172a;border-radius:12px;padding:16px;text-align:center;margin-top:24px;">
          <a href="https://scoutaibot.vercel.app" style="color:#22c55e;font-weight:bold;font-size:14px;">View All Predictions on ScoutAI →</a>
        </div>

        <p style="color:#94a3b8;font-size:11px;text-align:center;margin-top:16px;">
          You're receiving this because you subscribed on scoutaibot.vercel.app<br>
          ⚠ For entertainment only. Not financial advice. 18+ Gamble responsibly.
        </p>
      </body>
      </html>
    `;
    ...
    for (const sub of subs) {
      try {
        await resend.emails.send({
          from: "ScoutAI Daily Picks <onboarding@resend.dev>",
          to: sub.email,
          subject: `🔥 Today's Best Bet: ${potd.home} vs ${potd.away} — ScoutAI`,
          html,
        });
        sent++;
      } catch(e) { results.errors.push(`Email to ${sub.email}: ${e.message}`); }
    }
    results.emailsSent = sent;
  }

  res.json({ success: true, timestamp: new Date().toISOString(), subscriberCount: subs.length, ...results });
});

app.get("/", (req,res) => res.send("ScoutAI Server ✅ | Daily cron: GET /cron/daily"));
